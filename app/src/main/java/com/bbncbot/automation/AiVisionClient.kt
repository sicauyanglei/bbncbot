...
object AiVisionClient {
    ...
    private fun buildPrompt(sceneContext: String): String {
        return buildString {
            ...
            append("- CLICK_CLAIM  : 点击领取/确认按钮（领取奖励/领取肥料/我要直接拿奖励/直接拿奖励/立即拿奖励/点击跳转拿奖励/跳转拿奖励/签到/点击领取/立即领取/确定）领取奖励\n")
            append("- PRESS_BACK   : 按返回键（无法处理或应该退出当前页）\n")
            append("- SKIP_TASK    : 跳过当前任务（页面无肥料价值，如分享/评价/会员推销）\n")
            append("- WAIT         : 等待（页面正在加载或倒计时中，不应操作）\n\n")
            append("判断优先级：\n")
            append("1. 若有\"签到/点击领取/立即领取/恭喜获得/领取奖励/领取肥料/我要直接拿奖励/直接拿奖励/立即拿奖励/马上拿奖励/点击跳转拿奖励/跳转拿奖励/直接领取\"等领取按钮 → CLICK_CLAIM\n")
