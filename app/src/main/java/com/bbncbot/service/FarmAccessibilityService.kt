...
class FarmAccessibilityService : AccessibilityService() {
    ...
    fun isPaidTask(button: AccessibilityNodeInfo): Boolean {
        ...
        val paidKeywords = listOf(
            ...
            "投资", "理财", "保证金", "押金",             // 金融类
            // build658 修复（debug_test_20260726_210643.log, build657-b012ed9）：
            //   21:05:40 processTask: current task #1/7, text='去完成'
            //   task #1 文案 "攒一笔到余额宝当前进度0，总进度1 完成得500肥料 去完成"
            //   未识别为付费任务 → 点击"去完成"跳转余额宝转入页 → isNonAdTaskPage 误判
            //   → currentTaskIndex 重置无限循环 4 次直到 STOPPING
            // 修复：新增"余额宝"/"攒一笔"/"转入"关键词，识别为付费任务直接跳过。
            "余额宝", "攒一笔", "转入",
            ...
            "下单大额", "下单高额", "下单得大额",
            // build649: "淘宝秒杀下单肥料加码(0/1) 下单最高得4-5万肥料 +50000" 是付费任务
